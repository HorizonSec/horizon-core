...
class SensitiveDataFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        patterns = [
            ...
            (r"\b({})(\s+is\s+)(\S+)".format("|".join(self.SENSITIVE_KEYWORDS)), r"\1\2[REDACTED]"),
            # Pattern 2: "keyword: value" (e.g., "API key: abcdefghijklmnop")
            # \s* allows optional spaces around the colon
            (r"\b({})(\s*:\s+)(\S+)".format("|".join(self.SENSITIVE_KEYWORDS)), r"\1\2[REDACTED]"),
...
def setup_logger(name: str, level: int = logging.INFO, simple: bool = False) -> logging.Logger:
    ...
    if simple:
        ...
        formatter = SensitiveDataFormatter(DEFAULT_FORMAT)
